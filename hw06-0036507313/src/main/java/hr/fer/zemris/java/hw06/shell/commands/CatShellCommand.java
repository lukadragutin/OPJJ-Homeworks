...
public class CatShellCommand implements ShellCommand {
	...
	private final List<String> commandDescription = Collections
			.unmodifiableList(Arrays.asList(new String[] {
					"Writes the file from the path given as argument",
					"using the given charset for decoding. If no charset is",
					"given as argument, the system's default charset is used."
			}));
	
	/**
	 * {@inheritDoc}
	 * @throws IllegalArgumentException Ako su argumenti krivo zadani ili nepostojeći
	 */
	@Override
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		Path file = Paths.get(token.getValue());
