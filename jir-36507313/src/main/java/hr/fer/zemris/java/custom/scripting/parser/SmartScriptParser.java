...
public class SmartScriptParser {
	...
	private void parseTag(ScriptToken token) {
		...
		else if(token.getValue().toString().equalsIgnoreCase("NOW")) {
			nowTag(token);
		}
		else {
	...
	private void nowTag(ScriptToken token) {
		ScriptToken token2 = lexer.nextToken();
		TimeNode tNode;
		if(token2.getType() == ScriptTokenType.ENDTAG) {
			tNode = new TimeNode(null);
		}
		else if(token2.getType() == ScriptTokenType.STRING) {
			tNode = new TimeNode(token2.getValue().toString());
			if(lexer.nextToken().getType() != ScriptTokenType.ENDTAG) {
				throw new SmartScriptParserException();
			}
		}
		else {
			throw new SmartScriptParserException("Wrong type");
		}
		Node parentNode = stack.peek();
		parentNode.addChildNode(tNode);
	}
