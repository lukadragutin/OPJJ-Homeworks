...
/**
 * Simulira kretanje oko 3D scene te dinamicki racuna izgled
 * te scene iz razlicith tocki na sceni koje ovise o tocki iz koje se gleda, smjeru gledanja
 * te kutu pod kojim se gleda scena, a koji se mijenjaju kroz vrijeme nasumicno.
 * Izracuni se obavljaju visedretveno
...
public class RayCasterParallel2 {
	...
	/**
	 * Stvara i vraca objekt koji implementira sucelje {@link IRayTracerAnimator}
	 * @see IRayTracerAnimator
	 * @return objekt koji implementira sucelje {@link IRayTracerAnimator}
	 */
	...
	private static IRayTracerProducer getIRayTracerProducer() {
		return new IRayTracerProducer() {
			...
			public void produce(Point3D eye, Point3D view, Point3D viewUp, double horizontal, double vertical,
				...
				pool.invoke(new ColoringJob(0, height - 1, height, width, red, green, blue, vertical, horizontal,
	...
	public static class ColoringJob extends RecursiveAction {
		...
		public ColoringJob(int yMin, int yMax, int height, int width, short[] red, short[] green, short[] blue,
				double vertical, double horizontal, Point3D screenCorner, Point3D yAxis, Point3D xAxis, Point3D eye,
		...
		protected void compute() {
			...
			ColoringJob j1 = new ColoringJob(yMin, yMin + (yMax - yMin) / 2, height, width, red, green, blue, vertical,
					horizontal, screenCorner, yAxis, xAxis, eye, scene, cancel);
			ColoringJob j2 = new ColoringJob(1 + yMin + (yMax - yMin) / 2, yMax, height, width, red, green, blue,
					vertical, horizontal, screenCorner, yAxis, xAxis, eye, scene, cancel);
	...
	private static void determineColorFor(Scene scene, short[] rgb, RayIntersection intersection, Ray eyeRay) {
		...
		for (var light : scene.getLights()) {
			...
			if (close == null || close.getDistance() + 0.001 < distance) {
				continue;
			} else {
